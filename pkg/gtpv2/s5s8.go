...
func (c *K6GTPv2Client) SendCreateSessionRequestS5S8(daddr string, options S5S8SgwParams) (*gtpv2.Session, uint32, error) {
	...
	// todo v6
	var cteidIE *ie.IE
	if options.Cplaneteid == 0 {
		cteidIE = c.Conn.NewSenderFTEID(localIP, "")
	} else {
		cteidIE = ie.NewFullyQualifiedTEID(gtpv2.IFTypeS5S8SGWGTPC, options.Cplaneteid, localIP, "")
...
func (c *K6GTPv2Client) SendDeleteSessionRequestS5S8(options S5S8SgwParams) (uint32, error) {
	var err error
	s5Session, err := c.Conn.GetSessionByIMSI(options.Imsi)
	if err != nil {
		return 0, err
	}
	fmt.Println(s5Session)
	if options.Cplaneteid == 0 {
		options.Cplaneteid, err = s5Session.GetTEID(gtpv2.IFTypeS5S8PGWGTPC)
		if err != nil {
			return 0, err
		}
	}
...
func (c *K6GTPv2Client) CheckSendDeleteSessionRequestS5S8(options S5S8SgwParams) (bool, error) {
	seq, err := c.SendDeleteSessionRequestS5S8(options)
	if err != nil {
		return false, err
	}
